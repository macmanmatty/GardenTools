...
class UnitsTest {
    ...
    void convert_temperature_degC_to_degF() {
        // 0C = 32F
        double out = Units.convert(QuantityType.TEMPERATURE, 0.0, Unit.DEG_C, Unit.DEG_F);
    ...
    void convert_temperature_degF_to_K() {
        // 32F = 273.15K
        double out = Units.convert(QuantityType.TEMPERATURE, 32.0, Unit.DEG_F, Unit.K);
    ...
    void convert_wind_mps_to_mph() {
        // 1 m/s = 2.236936 mph
        double out = Units.convert(QuantityType.WIND_SPEED, 1.0, Unit.M_PER_S, Unit.MPH);
    ...
    void convert_pressure_hpa_to_kpa() {
        ...
        double out = Units.convert(QuantityType.PRESSURE, 1013.25, Unit.HPA, Unit.KPA);
    ...
    void convert_pressure_inhg_to_hpa() {
        ...
        double out = Units.convert(QuantityType.PRESSURE, 29.92, Unit.INHG, Unit.HPA);
    ...
    void convert_depth_mm_to_in() {
        // 25.4 mm = 1 inch
        double out = Units.convert(QuantityType.PRECIP_DEPTH, 25.4, Unit.MM, Unit.IN);
        assertEquals(1.0, out, EPS);
    }

    @Test
    void convert_returnsSameValueForSameUnit() {
        double out = Units.convert(QuantityType.PRESSURE, 123.456, Unit.HPA, Unit.HPA);
    ...
    void convert_throwsForUnsupportedQuantityType() {
        assertThrows(IllegalArgumentException.class, () ->
                Units.convert(QuantityType.RADIATION, 10.0, Unit.M, Unit.W_PER_M2)
        );
    }
    ...
    void convert_throwsForUnsupportedUnitPair() {
        ...
        assertThrows(IllegalArgumentException.class, () ->
                Units.convert(QuantityType.WIND_SPEED, 10.0, Unit.HPA, Unit.MPH)
        );
    }
}
